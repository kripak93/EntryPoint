...
def load_analytics(season_filter=None):
    ...
    # Method 2: Read directly from .env file (for local development)
    if not api_key and os.path.exists('.env'):
        try:
            ...
            pass  # Silent fail, will try Streamlit secrets
    ...
    if not api_key or api_key == 'your_gemini_api_key_here' or api_key == 'your_actual_api_key_here':
        ...
        st.info("Please add your Gemini API key in .env file (local) or Streamlit secrets (cloud)")
    ...
    try:
        ...
        st.sidebar.success(f"✅ AI Model: {analytics.model_name}")
        st.sidebar.info(f"📅 Season: {analytics.season}")
        return analytics
    except Exception as e:
        st.error(f"Failed to initialize analytics: {str(e)}")
        st.info("Check your API key and data file")
        st.stop()
...
with tab4:
    ...
    if prep_type == "Batsman Scouting":
        ...
        if selected_batsman and selected_bowler_type:
            ...
            if not batsman_data.empty:
                ...
                st.info(f"📊 {selected_batsman} has {total_balls} balls in dataset")
            else:
                st.warning(f"⚠️ No data found for {selected_batsman}")
        ...
        if st.button("🎯 Generate Scouting Brief", type="primary"):
            with st.spinner(f"Analyzing {selected_batsman} vs {selected_bowler_type}..."):
                try:
                    ...
                    if brief.startswith("❌"):
                        st.error(brief)
                        st.info("Try reducing minimum balls or selecting a different player/season")
                    else:
