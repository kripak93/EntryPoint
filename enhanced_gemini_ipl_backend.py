...
class EnhancedGeminiIPLAnalytics:
    ...
    def __init__(self, csv_path, api_key=None, model_name='gemini-2.5-flash', season_filter=None):
        ...
        if 'Match⬆' in self.df.columns:
            ...
            # Safety check - ensure we still have data after filtering
            if len(self.df) == 0:
                raise ValueError("No data remaining after filtering for Men's IPL (LAT20). Check your dataset.")
        ...
        if season_filter and 'Date⬆' in self.df.columns:
            try:
                ...
                # Safety check for season filtering
                if len(self.df) == 0:
                    raise ValueError(f"No data found for season {season_filter}. Available seasons: {sorted(pd.to_datetime(pd.read_csv(csv_path)['Date⬆']).dt.year.unique())}")
                
                self.season = season_filter
            except Exception as e:
                print(f"Warning: Season filtering failed: {e}")
                self.season = "All seasons (season filter failed)"
        else:
...
"""
    ...
    def get_player_insights(self, player_name):
        ...
        if not batting_stats.empty:
            total_runs = batting_stats['R.1'].max()  # Max cumulative runs
            total_balls = len(batting_stats)  # Total balls faced
            strike_rate = (total_runs / total_balls * 100) if total_balls > 0 else 0
            
            # Get match-wise performance
            matches_played = batting_stats['Match⬆'].nunique() if 'Match⬆' in batting_stats.columns else 'Unknown'
            
            # Boundary analysis
            fours = len(batting_stats[batting_stats['4'] == 1]) if '4' in batting_stats.columns else 0
            sixes = len(batting_stats[batting_stats['6'] == 1]) if '6' in batting_stats.columns else 0
            dots = len(batting_stats[batting_stats['0'] == 1]) if '0' in batting_stats.columns else 0
            
            batting_summary = f"""
COMPREHENSIVE BATTING ANALYSIS:
- Total Records: {len(batting_stats)} balls faced
- Matches Played: {matches_played}
- Total Runs: {total_runs}
- Strike Rate: {strike_rate:.1f}
- Boundaries: {fours} fours, {sixes} sixes
- Dot Balls: {dots} ({dots/len(batting_stats)*100:.1f}%)
- Teams: {', '.join(batting_stats['Team.1'].unique())}

RECENT PERFORMANCE (Last 10 balls):
{batting_stats[['Batsman', 'Team.1', 'R.1', 'B', 'RR', 'Overs']].tail(10).to_string()}
"""
        else:
            batting_summary = "No batting data available"
        
        if not bowling_stats.empty:
            total_overs = bowling_stats['O'].sum()
            total_wickets = bowling_stats['W'].sum()
            total_runs_conceded = bowling_stats['R'].sum()
            
            bowling_summary = f"""
COMPREHENSIVE BOWLING ANALYSIS:
- Total Records: {len(bowling_stats)} balls bowled
- Total Overs: {total_overs}
- Total Wickets: {total_wickets}
- Total Runs Conceded: {total_runs_conceded}
- Economy Rate: {(total_runs_conceded/total_overs):.2f} (overall)
- Teams: {', '.join(bowling_stats['Team'].unique())}

RECENT PERFORMANCE (Last 10 balls):
{bowling_stats[['Player', 'Team', 'O', 'M', 'R', 'W', 'Econ']].tail(10).to_string()}
"""
        else:
            bowling_summary = "No bowling data available"
...
CRITICAL: This analysis is based on {len(player_data)} total records in the dataset.

Provide:
1. Role assessment (bowler/batsman/all-rounder)
2. Key performance metrics based on the comprehensive data above
